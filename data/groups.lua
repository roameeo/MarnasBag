...
---@param categoryName string
---@return string
function groups:GetMainBackpackGroupKeyForCategory(categoryName)
  local name = strlower(categoryName or "")

  if strfind(name, "quest", 1, true) then
    return "quest"
  end

  if strfind(name, "junk", 1, true) or strfind(name, "trash", 1, true) then
    return "junk"
  end

  if strfind(name, "housing", 1, true) or strfind(name, "toy", 1, true) or strfind(name, "furniture", 1, true) or strfind(name, "decor", 1, true) then
    return "housing"
  end

  if strfind(name, "craft", 1, true) or strfind(name, "profession", 1, true) or strfind(name, "reagent", 1, true) or
     strfind(name, "alchemy", 1, true) or strfind(name, "blacksmith", 1, true) or strfind(name, "enchant", 1, true) or
     strfind(name, "engineering", 1, true) or strfind(name, "inscription", 1, true) or strfind(name, "jewel", 1, true) or
     strfind(name, "leatherworking", 1, true) or strfind(name, "tailor", 1, true) then
    return "crafting"
  end
...
function groups:CategoryBelongsToGroup(kind, categoryName, groupID)
  if kind == const.BAG_KIND.BACKPACK and database:GetUseMainCategoryTabs() then
    local assignedGroup = database:GetCategoryGroup(kind, categoryName)
    if assignedGroup ~= nil then
      return assignedGroup == groupID
    end

    local groupKey = self:GetMainBackpackGroupKeyForCategory(categoryName)
    local mappedGroupID = database:GetBackpackMainGroupID(groupKey)
