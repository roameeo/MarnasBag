...
function backpack.proto:RegisterEvents()
	...
	events:RegisterMessage("groups/TabPositionChanged", function(ctx)
		if behavior.bag and behavior.bag.tabs then
			behavior.bag.tabs:ApplyLayout()
			behavior.bag.tabs:ResizeAllTabs(ctx)
		end
	end)
end
...
function backpack.proto:GenerateGroupTabs(ctx)
	...
	self.bag.tabs:ApplyLayout()

	local allGroups = groups:GetAllGroups(const.BAG_KIND.BACKPACK)
	local useMainTabs = database:GetUseMainCategoryTabs()
	local visibleGroups = {}

	if useMainTabs then
		for _, groupDef in ipairs(const.MAIN_BACKPACK_GROUPS) do
			local groupID = database:GetBackpackMainGroupID(groupDef.key)
			if groupID and allGroups[groupID] then
				allGroups[groupID].name = groupDef.name
				allGroups[groupID].order = allGroups[groupID].order or 0
				visibleGroups[groupID] = true
			end
		end
	else
		for groupID in pairs(allGroups) do
			visibleGroups[groupID] = true
		end
	end
	...
	-- Hide tabs for groups that are no longer visible (skip the "+" tab with ID 0)
	for _, tab in pairs(self.bag.tabs.tabIndex) do
		if tab.id and tab.id > 0 and (not allGroups[tab.id] or not visibleGroups[tab.id]) then
			self.bag.tabs:HideTabByID(tab.id)
		end
	end

	-- Always keep "+" tab for creating additional groups
	if not self.bag.tabs:TabExistsByID(NEW_GROUP_TAB_ID) then
		...
		self.bag.tabs:ShowTabByID(NEW_GROUP_TAB_ID)
	end

	if useMainTabs then
		for _, groupDef in ipairs(const.MAIN_BACKPACK_GROUPS) do
			local groupID = database:GetBackpackMainGroupID(groupDef.key)
			if groupID and self.bag.tabs:TabExistsByID(groupID) then
				self.bag.tabs:MoveToEnd(groupDef.name)
			end
		end
	...
	local activeGroup = database:GetActiveGroup(const.BAG_KIND.BACKPACK)
	if not activeGroup or not visibleGroups[activeGroup] then
		if useMainTabs then
			activeGroup = database:GetBackpackMainGroupID("equipment") or 1
		else
			activeGroup = 1
		end
		database:SetActiveGroup(const.BAG_KIND.BACKPACK, activeGroup)
	end
...
function backpack.proto:OnTabClicked(ctx, tabID, button)
	...
	if button == "RightButton" and tabID then
		if database:GetUseMainCategoryTabs() and database:IsBackpackMainGroup(tabID) then
			return false
		end
