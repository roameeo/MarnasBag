...
local searchCategoryConfig = addon:GetModule('SearchCategoryConfig', true)
...
function categoryPaneProto:LoadPinnedItems()
  local filterText = strlower(strtrim(self.categoryFilterText or ""))
  ...
  for _, element in ipairs(sortedList) do
    if not hasFilter or strfind(strlower(element.title), filterText, 1, true) then
...
function categoryPaneProto:ShowCreateCategoryDialog()
  if not StaticPopupDialogs["BETTERBAGS_CREATE_CATEGORY"] then
    StaticPopupDialogs["BETTERBAGS_CREATE_CATEGORY"] = {
      ...
      button2 = "Cancel",
      OnAccept = function(f)
        local newName = strtrim(f.EditBox:GetText() or "")
        if newName == "" then
          print("Category name cannot be empty.")
          return
        end
        if categories:DoesCategoryExist(newName) then
          print(format("A category named '%s' already exists."), newName)
          return
        end

        local ctx = context:New('CategoryPane_CreateManualCategory')
        ...
        if f.data and f.data.pane then
          local pane = f.data.pane
          pane:RefreshList()
          pane:SelectCategory(newName)
        end
      end,
      OnShow = function(f)
        f.EditBox:SetText("")
      ...
      EditBoxOnEnterPressed = function(f)
        ...
        local newName = strtrim(parent.EditBox:GetText() or "")
        if newName == "" then
          print("Category name cannot be empty.")
          parent:Hide()
          return
        ...
        if parent.data and parent.data.pane then
          local pane = parent.data.pane
          pane:RefreshList()
          pane:SelectCategory(newName)
        end
        parent:Hide()
...
function categoryPaneProto:ShowCreateSearchCategoryDialog()
  if not searchCategoryConfig or not searchCategoryConfig.Open then
    print("Search category UI is not available.")
    return
  end

  local initialQuery = ""
  if self.categoryFilterBox then
    initialQuery = strtrim(self.categoryFilterBox:GetText() or "")
  end
  ...
  searchCategoryConfig:Open({
    ...
    searchCategory = {
      query = initialQuery,
      groupBy = const.SEARCH_CATEGORY_GROUP_BY.NONE,
    },
  }, self.frame)
end
...
function categoryPaneProto:RefreshList()
  ...
  for _, name in ipairs(sortedNames) do
    if (not hasFilter or strfind(strlower(name), filterText, 1, true)) and not pinnedList[name] then
...
function categoryPane:Create(parent, kind)
  ...
  local listToolbar = CreateFrame("Frame", nil, listContainer)
  listToolbar:SetPoint("TOPLEFT", 0, 0)
  listToolbar:SetPoint("TOPRIGHT", -18, 0)
  listToolbar:SetHeight(56)

  local categoryFilterBox = CreateFrame("EditBox", nil, listToolbar, "InputBoxTemplate")
  categoryFilterBox:SetPoint("TOPLEFT", 6, -6)
  categoryFilterBox:SetPoint("TOPRIGHT", -6, -6)
  categoryFilterBox:SetHeight(20)
  categoryFilterBox:SetAutoFocus(false)
  categoryFilterBox:SetMaxLetters(100)
  categoryFilterBox:SetScript("OnTextChanged", function()
    pane.categoryFilterText = categoryFilterBox:GetText() or ""
    pane:RefreshList()
  end)

  local manualButton = CreateFrame("Button", nil, listToolbar, "UIPanelButtonTemplate")
  manualButton:SetPoint("TOPLEFT", categoryFilterBox, "BOTTOMLEFT", 0, -5)
  manualButton:SetSize(92, 20)
  manualButton:SetText("+ Category")
  manualButton:SetScript("OnClick", function()
    pane:ShowCreateCategoryDialog()
  end)

  local searchButton = CreateFrame("Button", nil, listToolbar, "UIPanelButtonTemplate")
  searchButton:SetPoint("LEFT", manualButton, "RIGHT", 6, 0)
  searchButton:SetSize(92, 20)
  searchButton:SetText("+ Search")
  searchButton:SetScript("OnClick", function()
    pane:ShowCreateSearchCategoryDialog()
  end)

  pane.categoryFilterBox = categoryFilterBox
