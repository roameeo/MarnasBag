...
function config:CreateConfig()
  ...
  local function UpdateBackpackSidePositionControl()
    local isSide = db:GetBackpackTabPosition() == const.BACKPACK_TAB_POSITION.SIDE
    local dropdowns = rawget(f.layout, "dropdowns")
    if type(dropdowns) ~= "table" then
      return
    end

    local enableClassicDropdown = _G["UIDropDownMenu_EnableDropDown"]
    local disableClassicDropdown = _G["UIDropDownMenu_DisableDropDown"]

    for container in pairs(dropdowns) do
      if container.title and container.title:GetText() == 'Backpack Side Position' then
        if addon.isRetail and container.dropdown then
          if isSide then
            if container.dropdown.Enable then
              container.dropdown:Enable()
            end
            container.dropdown:SetAlpha(1)
          else
            if container.dropdown.Disable then
              container.dropdown:Disable()
            end
            container.dropdown:SetAlpha(0.5)
          end
        elseif container.classicDropdown then
          if isSide then
            if type(enableClassicDropdown) == "function" then
              enableClassicDropdown(container.classicDropdown)
            end
            container.classicDropdown:SetAlpha(1)
          else
            if type(disableClassicDropdown) == "function" then
              disableClassicDropdown(container.classicDropdown)
            end
            container.classicDropdown:SetAlpha(0.5)
          end
        end
        break
      end
 ...
 f:AddSection({
  ...
  for _, bagType in ipairs(bagTypes) do
    ...
    if bagType.kind == const.BAG_KIND.BACKPACK then
      ...
      f:AddCheckbox({
        title = 'Use Main Category Tabs',
        description = 'Use fixed backpack tabs for Equipment, Quest items, Crafting items, Housing items, and Junk. Keep enabled for the curated tabbed view.',
        getValue = function(_)
          return db:GetUseMainCategoryTabs()
        end,
        setValue = function(ctx, value)
          db:SetUseMainCategoryTabs(value)
          events:SendMessage(ctx, 'groups/Changed', nil, nil, nil, bagType.kind)
          events:SendMessage(ctx, 'bags/FullRefreshAll')
        end
      })

      f:AddDropdown({
        title = 'Backpack Tab Position',
        description = 'Choose where backpack tabs are shown.',
        items = {'Top', 'Side'},
        getValue = function(_, value)
          local position = db:GetBackpackTabPosition()
          return (value == 'Top' and position == const.BACKPACK_TAB_POSITION.TOP) or
                 (value == 'Side' and position == const.BACKPACK_TAB_POSITION.SIDE)
        end,
        setValue = function(ctx, value)
          local position = value == 'Side' and const.BACKPACK_TAB_POSITION.SIDE or const.BACKPACK_TAB_POSITION.TOP
          db:SetBackpackTabPosition(position)

          if position == const.BACKPACK_TAB_POSITION.TOP and not db:GetRememberBackpackSidePosition() then
            db:SetBackpackSidePosition(const.BACKPACK_SIDE_POSITION.RIGHT)
          end

          UpdateBackpackSidePositionControl()
          events:SendMessage(ctx, 'groups/TabPositionChanged')
          events:SendMessage(ctx, 'bags/FullRefreshAll')
        end,
      })
      ...
      f:AddCheckbox({
        ...
        setValue = function(ctx, value)
          db:SetRememberBackpackSidePosition(value)
          if not value and db:GetBackpackTabPosition() == const.BACKPACK_TAB_POSITION.TOP then
            db:SetBackpackSidePosition(const.BACKPACK_SIDE_POSITION.RIGHT)
          end
          UpdateBackpackSidePositionControl()
          events:SendMessage(ctx, 'groups/TabPositionChanged')
      ...
      f:AddDropdown({
        title = 'Backpack Side Position',
        description = 'When Backpack Tab Position is set to Side, choose which side to place tabs on.',
        items = {'Left', 'Right'},
        getValue = function(_, value)
          local side = db:GetBackpackSidePosition()
          return (value == 'Left' and side == const.BACKPACK_SIDE_POSITION.LEFT) or
                 (value == 'Right' and side == const.BACKPACK_SIDE_POSITION.RIGHT)
        end,
        setValue = function(ctx, value)
          local side = value == 'Left' and const.BACKPACK_SIDE_POSITION.LEFT or const.BACKPACK_SIDE_POSITION.RIGHT
          db:SetBackpackSidePosition(side)
          UpdateBackpackSidePositionControl()
          events:SendMessage(ctx, 'groups/TabPositionChanged')
          events:SendMessage(ctx, 'bags/FullRefreshAll')
        end,
      ...
      bucket:Later("updateBackpackSidePositionControl", 0.1, function()
        UpdateBackpackSidePositionControl()
      end)
    end
