...
---@return boolean
function DB:GetUseMainCategoryTabs()
  if DB.data.profile.useMainCategoryTabs == nil then
    return true
  end
  return DB.data.profile.useMainCategoryTabs
end

---@param value boolean
function DB:SetUseMainCategoryTabs(value)
  DB.data.profile.useMainCategoryTabs = value
end

---@return BackpackTabPosition
function DB:GetBackpackTabPosition()
  return DB.data.profile.backpackTabPosition or const.BACKPACK_TAB_POSITION.TOP
end

---@param value BackpackTabPosition
function DB:SetBackpackTabPosition(value)
  DB.data.profile.backpackTabPosition = value
end

---@return BackpackSidePosition
function DB:GetBackpackSidePosition()
  return DB.data.profile.backpackSidePosition or const.BACKPACK_SIDE_POSITION.RIGHT
end

---@param value BackpackSidePosition
function DB:SetBackpackSidePosition(value)
  DB.data.profile.backpackSidePosition = value
end

---@return boolean
function DB:GetRememberBackpackSidePosition()
  if DB.data.profile.rememberBackpackSidePosition == nil then
    return true
  end
  return DB.data.profile.rememberBackpackSidePosition
end

---@param value boolean
function DB:SetRememberBackpackSidePosition(value)
  DB.data.profile.rememberBackpackSidePosition = value
end
...
---@return table<string, number>
function DB:GetMainBackpackGroups()
  DB.data.profile.mainBackpackGroups = DB.data.profile.mainBackpackGroups or {}
  return DB.data.profile.mainBackpackGroups
end

---@param key string
---@return number?
function DB:GetBackpackMainGroupID(key)
  local mappings = self:GetMainBackpackGroups()
  return mappings[key]
end

---@param key string
---@param groupID number
function DB:SetBackpackMainGroupID(key, groupID)
  local mappings = self:GetMainBackpackGroups()
  mappings[key] = groupID
end

---@param groupID number
---@return boolean
function DB:IsBackpackMainGroup(groupID)
  local mappings = self:GetMainBackpackGroups()
  for _, id in pairs(mappings) do
    if id == groupID then
      return true
    end
  end
...
function DB:Migrate()
  ...
  if DB.data.profile.useMainCategoryTabs == nil then
    DB.data.profile.useMainCategoryTabs = true
  end
  if DB.data.profile.backpackTabPosition == nil then
    DB.data.profile.backpackTabPosition = const.BACKPACK_TAB_POSITION.TOP
  end
  if DB.data.profile.backpackSidePosition == nil then
    DB.data.profile.backpackSidePosition = const.BACKPACK_SIDE_POSITION.RIGHT
  end
  if DB.data.profile.rememberBackpackSidePosition == nil then
    DB.data.profile.rememberBackpackSidePosition = true
  end
  if DB.data.profile.mainBackpackGroups == nil then
    DB.data.profile.mainBackpackGroups = {}
  end

  local function EnsureMainBackpackGroups()
    local backpackGroups = DB.data.profile.groups[const.BAG_KIND.BACKPACK]
    local mappings = DB.data.profile.mainBackpackGroups

    local function FindGroupIDByName(name)
      local target = strlower(name)
      for id, group in pairs(backpackGroups) do
        if type(group) == "table" and group.name and strlower(group.name) == target then
          return id
        end
      end
      return nil
    ...
    for order, groupDef in ipairs(const.MAIN_BACKPACK_GROUPS) do
      local id = mappings[groupDef.key]
      if not id or not backpackGroups[id] then
        id = FindGroupIDByName(groupDef.name)
      end

      if not id then
        local newID = DB.data.profile.groupCounter[const.BAG_KIND.BACKPACK] + 1
        DB.data.profile.groupCounter[const.BAG_KIND.BACKPACK] = newID
        backpackGroups[newID] = {
          id = newID,
          name = groupDef.name,
          order = order,
          kind = const.BAG_KIND.BACKPACK,
          isDefault = groupDef.key == "equipment",
        }
        id = newID
      else
        local group = backpackGroups[id]
        group.name = groupDef.name
        group.order = order
        group.kind = const.BAG_KIND.BACKPACK
        group.isDefault = groupDef.key == "equipment"
        group.bankType = nil
      end

      mappings[groupDef.key] = id
    end

    local activeGroup = DB.data.profile.activeGroup[const.BAG_KIND.BACKPACK]
    if not activeGroup or not backpackGroups[activeGroup] or not DB:IsBackpackMainGroup(activeGroup) then
      local equipmentID = mappings.equipment
      if equipmentID then
        DB.data.profile.activeGroup[const.BAG_KIND.BACKPACK] = equipmentID
      end
    end
  end

  if DB.data.profile.useMainCategoryTabs then
    EnsureMainBackpackGroups()
  end
