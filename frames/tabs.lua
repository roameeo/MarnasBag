...
function tabFrame:ApplyLayout()
	self.frame:ClearAllPoints()

	local isSideLayout = self.kind == const.BAG_KIND.BACKPACK and database:GetBackpackTabPosition() == const.BACKPACK_TAB_POSITION.SIDE
	if isSideLayout then
		self.layoutPosition = const.BACKPACK_TAB_POSITION.SIDE
		local sidePosition = database:GetBackpackSidePosition()
		if sidePosition == const.BACKPACK_SIDE_POSITION.LEFT then
			self.frame:SetPoint("TOPRIGHT", self.parentFrame, "TOPLEFT", -8, 0)
			self.frame:SetPoint("BOTTOMRIGHT", self.parentFrame, "BOTTOMLEFT", -8, 0)
		else
			self.frame:SetPoint("TOPLEFT", self.parentFrame, "TOPRIGHT", 8, 0)
			self.frame:SetPoint("BOTTOMLEFT", self.parentFrame, "BOTTOMRIGHT", 8, 0)
		end
		self.frame:SetWidth(140)
		self.frame:SetHeight(1)
	else
		self.layoutPosition = const.BACKPACK_TAB_POSITION.TOP
		self.frame:SetPoint("TOPLEFT", self.parentFrame, "BOTTOMLEFT", 0, 2)
		self.frame:SetPoint("TOPRIGHT", self.parentFrame, "TOPRIGHT", 0, 2)
...
function tabFrame:ReanchorTabs()
	...
	if self.layoutPosition == const.BACKPACK_TAB_POSITION.SIDE then
		local currentY = -2
		local maxWidth = 0
		for _, tab in ipairs(visibleTabs) do
			tab:ClearAllPoints()
			tab:SetPoint("TOPLEFT", self.frame, "TOPLEFT", 0, currentY)
			currentY = currentY - (tab:GetHeight() + 4)
			if tab:GetWidth() > maxWidth then
				maxWidth = tab:GetWidth()
			end
		end
		self.width = maxWidth
		if maxWidth > 0 then
			self.frame:SetWidth(maxWidth + 6)
		end
	else
...
function tabs:Create(parent, kind)
	...
	container.layoutPosition = const.BACKPACK_TAB_POSITION.TOP
...
function tabs:IsTabReorderable(kind, tab)
	if kind == const.BAG_KIND.BACKPACK and database:GetBackpackTabPosition() == const.BACKPACK_TAB_POSITION.SIDE then
		return false
	end
